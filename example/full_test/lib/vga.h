#define VRAM_BASE_ADDR 0x400000  // the start address of VRAM
...
#define SCALE_RATIO \
  8  // the scale ratio of the screen, 8 means 1 pixel in VRAM is 8x8 pixels on
     // the screen
#define VRAM_WIDTH \
  128  // (SCREEN_WIDTH / SCALE_RATIO)    // the width of VRAM: 128
#define VRAM_HEIGHT \
  96  // (SCREEN_HEIGHT / SCALE_RATIO)  // the height of VRAM: 96
#define VRAM_SIZE 12288

#define VRAM_OFFSET(x, y) \
  ((x) * VRAM_WIDTH + (y))  // get the address offset relative to VRAM_ADDR
